...
from Funcoes_API_Novo import cria_pastas_rodada
...
from Funcoes_API_Novo import renomeia_prevs
from Funcoes_API_Novo import limpa_prevs_semana_atual
from Funcoes_API_Novo import cria_rodada
from Funcoes_API_Novo import gera_arquivo_UH_atualizado
from Funcoes_API_Novo import copia_arquivos_padrão_rodada_diaria
from Funcoes_API_Novo import download_estudos_finalizados_diario2
from Funcoes_API_Novo import processa_rodada_diaria
from Funcoes_API_Novo import arquiva_dados_rodada_diaria
import datetime
import time
import os
...
import logging
from Funcoes_API_Novo import envia_email_python
...
logging.basicConfig(filename=caminho_log + '\\' + nome_log, format='%(asctime)s - %(message)s',
...
try:
    while a:
        ...
        if now < hora_limite:
            ...
            x = False
        print('Iniciando verificação de disponibilidade de PREVS (', pasta_relatorio, ') - data e hora: ',
    ...
    logging.info(
        'Pastas criadas na rede, iniciando coleta de arquivos padrão da semana (deck NEWAVE, deck DECOMP e Arquivos GEVAZP).')
    ...
    for arquivo in os.listdir(caminho_local):  # lê os arquivos que estão na pasta de download
        ...
        if arquivo.upper().endswith('-PREVS.ZIP'):  # exibe somente os arquvos de ENA
           ...
           # shutil.move(Path.joinpath(Path(caminho_local), nome_pasta),
           #             Path.joinpath(Path(caminho_rodada_prevs), nome_pasta))
    ...
    for folder in os.listdir(caminho_rodada_prevs):
        ...
        origem = os.path.join(caminho_rodada_prevs, folder)
        prevsM1 = r'C:\Users\alex.lourenco\OneDrive - Eneva S.A\Documentos\processos_alex\diario\02.Prevs\prevsM1'
            #r'J:\SEDE\Comercializadora de Energia\6. MIDDLE\13.RODADAS\05. Diario\05.Arquivos Semanais\04.Prevs\prevsM1'  # new
        if folder != 'prevs':
            for file in os.listdir(origem):
    ...
    caminho_arquivo_criacao_estudo = r'C:\Users\alex.lourenco\OneDrive - Eneva S.A\Documentos\processos_alex\diario\Criacao_Estudos_diario.xlsm'
        #r'J:\SEDE\Comercializadora de Energia\6. MIDDLE\13.RODADAS\05. Diario\Criacao_Estudos_diario.xlsm'
    ...
    cria_rodada(caminho_arquivo_criacao_estudo, Diario=pasta_relatorio, Caminho_rodada=caminho, Executa_rodada='True',
                Envia_volume_UHE='False')
    ...
    while True:
        ...
        resposta = download_estudos_finalizados_diario2(
            pasta_relatorio)  # baixa estudos, salva na rede, descompacta, joga na base prospec
...
#    logging.info('E-mail com base de dados do prospec enviado para ativação do fluxo do Power Automate')
    ...
    assunto = 'ERRO DE CÓDIGO NO BOL. DIÁRIO - ' + pasta_relatorio + ' - ' + datetime.datetime.today().strftime(
        '%d/%m/%Y')
