...
def downloadForecast(idForecast, pathToDownload, fileName):
    response = getFileFromAPI(token, '/v2/resultados/' + str(idForecast), fileName, pathToDownload)
# -----------------------------------------------------------------------------
...
def getIdsOfForecastModels():
    return getInfoFromAPI(token, '/v2/valoresParametros/modelos')
...
def getIdsOfPrecipitationsDataSource():
    return getInfoFromAPI(token, '/v2/valoresParametros/mapas')
...
def getForecasts(forecastDate, forecastSources, forecastModels, bias, preliminary, years, members, modes):
    ...
    if bias != '':
        params += "&Vies="+str(bias) #str.lower(bias)
    if preliminary != '':
    ...
    #return params
    # print(params)
    return getInfoFromAPI(token, '/v2/previsoes?' + params)
# -----------------------------------------------------------------------------
...
def le_ena_pasta_bacias(caminho_pasta_real_path):
    #falta inserir o nome do
    dia = int(caminho_pasta_real_path[-6:-4])
    mes = int(caminho_pasta_real_path[-8:][:-6])
    ano = int(caminho_pasta_real_path[-10:][:-8])
    data = datetime.datetime(ano, mes, dia).strftime('%d/%m/%Y')
    ...
    for previsao in lista_mapas:
        ...
        if forecasts == []:
            ...
            for forecast in forecasts:
                ...
                if forecast['preliminar'] == 'definitiva':
                    prelim = 'definitiva'
...
def verifica_disponibilidade_PREVS (data_mapas, **kwargs):
    ...
    if kwargs.get('Preliminar'):
        prelim_var = 'preliminar'
    else:
        prelim_var = 'definitiva'
