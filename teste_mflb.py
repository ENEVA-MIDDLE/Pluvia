...
caminho_onedrive = r'C:\Users\Middle\OneDrive - Eneva S.A\4.PrevPluvia_bacia\Base_ENA_Pluvia_bacias.xlsx'
caminho_local_raiz =  r'C:\Users\Middle\Documents'
...
if agora >= hora_limite_definitivo:
    preliminary = 'False'
    envia_email = True###############mudei_temporariamente
else:
    preliminary = 'True'
    envia_email = True
...
#Até aqui só baixou os arquivos selecionado no Pluvia
...
if download_arquivo: #só verifica a pasta se houver download de arquivos
    ...
    #Pega a ultima base atualizada na rede e joga em uma caminho local com '_temp' no final
    ...
    time.sleep(5)
    #Abre a base e deleta linha duplicadas
    #VOLTARdeleta_linhas_duplicadas_data(hoje, caminho_arquivo_temp) 
    for arquivo in os.listdir(pathForecastDay): #lê os arquivos que estão na pasta de download
        if arquivo.upper().endswith('ENA.ZIP'): #exibe somente os arquvos de ENA
            ...
            membro = tabela[0][3]
            print(tabela)
            #tabela.to_excel (caminho_local_raiz + r'\tabela.xlsx', index = False, header=True)##########
            ...
            shutil.rmtree(caminho_pasta)#remove o ziper
    ...
    shutil.rmtree(pathForecastDay)#deleta as pastas unzipadas
...
#    #caminho_gif = verifica_mapas_ONS_baixados ()
    destinatario = ['maria.barbosa@eneva.com.br'] #todosenevacom

#    #envia_email_pdf_gif (assunto, corpo, caminho_pdf, caminho_gif, destinatario)
    envia_email_python(assunto, corpo, destinatario)
