...
class SQL(object):
    ...
    def __init__(self, host: str = None, port: int = None, user: str = None, password: str = None, database: str = None,
        ...
        self.__host = environ.get('SQL_HOST', host)
        self.__port = environ.get('SQL_PORT', port)
        self.__user = environ.get('SQL_USERNAME', user)
        self.__password = environ.get('SQL_PASSWORD', password)
        self.__database = environ.get('SQL_PASSWORD', database)
        ...
        assert self.__database, 'database must be informed on instance or by environment variable'
    ...
    def __connect(self):
        ...
        self.__conn = pyodbc.connect(host=self.__host,
                                       port=self.__port,
    ...
    def bulk_insert(self, table: str, data: str, columns: tuple, sep: str = '\t', null_value: str = 'None') -> None:
        ...
        try:
            ...
            if float(psycopg2.__version__[:3]) >= 2.9:
                sql = 'COPY {}({}) FROM STDIN  WITH NULL AS'.format(
                    table, ','.join(columns)) + " '{}' DELIMITER as '{}'".format(null_value, sep)
